...
# =====================================================
def usage():
    print "updatePathsInXML.py - A tool for updating all Unbeast specificiation XML files in the current directory in order to contain correct paths to the syntax validation DTD file and the LTL-to-Büchi tool. All XML-files in the current directory should be Unbeast specifications"
    print "(C) 2010 by Ruediger Ehlers."
    print ""
    print "Usage:"
    print "  /path/to/updatePathsInXML.py <path/to/ltl-to-buechitool/tool> <ltl-to-buechitool parameters>"
    print ""
...
# =====================================================
# Try to read & write the XML files
# =====================================================
for filename in glob.glob(os.path.join('*.xml')):
    try:
    ...
    # Then replace LTL-to-Büchi tool in XML file
    replacedPath = False
                ...
                if basicNode.nodeType == basicNode.ELEMENT_NODE:
                    if basicNode.nodeName=="PathToLTLCompiler":
                        ...
                        if basicNode.childNodes[0].nodeType!=basicNode.TEXT_NODE:
                            ...
                            sys.exit(1)
                        replacedPath = True
                        basicNode.childNodes[0].nodeValue = " ".join(sys.argv[1:])

    # =====================================================
    # Write XML to file
    # =====================================================
    print "Updating XML file: "+filename
