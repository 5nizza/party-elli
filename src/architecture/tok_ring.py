...
class TokRingArchitecture:
    ...
    def get_cutoff(self, property:SpecProperty):
        _assert_no_single_concrete(property.assumptions) #not sure that works with single .. => assertion, just in case
    ...
    def get_async_hub_assumptions(self, has_tok_base_name:str, sends_prev_base_name:str):
        """ Return Forall(i) G(tok_i -> !sends_prev_i), Forall(i) G(!tok_i -> Fsends_prev_i)
        """
        # TODO: the second assumption is likely to be implied by the implications.
