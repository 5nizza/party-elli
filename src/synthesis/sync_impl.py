...
#            init_process_states = [self._get_state_name(sys_state_type, 1), self._get_state_name(sys_state_type, 0)]
        ...
        self.outvar_desc_by_process = [self._build_desc_by_outvar(all_output_signals, archi_outputs, self.orig_inputs[0])]
    ...
    def _get_tok_rings_safety_props(self):
        ...
        for state in states:
            ...
            tok_dont_disappear = forall_bool(free_vars,
                op_implies(op_and([has_tok_expr, op_not(sends_tok_expr)]), tok_of_tau_nprev_expr))
            ...
            no_sends_prev_no_tok_means_no_next_tok = forall_bool(free_vars,
                op_implies(op_not(has_tok_expr), op_not(tok_of_tau_nprev_expr)))
