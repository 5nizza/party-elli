...
def _get_spec(spec_type):
    ...
    specs = [
        (inputs, outputs, 'true', 'G(r_i->Fg_i)', mutual_exclusion),
...
#    par_fair_tokens = 'GF{tok}i'.format(tok=HAS_TOK_NAME)
#    par_fair_tokens = 'G((!{tok}i) -> F{prev}i) && G(!({prev}i && {tok}i))'.format(
#        tok=HAS_TOK_NAME, prev=SENDS_PREV_NAME)
...
if __name__ == '__main__':
    ...
    main({'pnueli': pnueli, 'full':full, 'simple':simple}[args.ltl],
