...
from syntcomp.aiger_synthesizer import synthesize as aiger_synthesize
...
def _synthesize(spec, is_moore, ltl_to_atm, min_k, max_k) -> str or None:
    ...
    for k in range(min_k, max_k+1, 2):
        ...
        model_aiger = aiger_synthesize(aiger_spec, is_moore, BAD_OUT_NAME)
        if model_aiger:
            return model_aiger
    return model_aiger


def synthesize_unreal(ltl_text:str, part_text:str, is_moore:bool,
    ...
    return _synthesize(neg_spec, neg_is_moore, ltl_to_atm, min_k, max_k)


def synthesize_real(ltl_text:str, part_text:str, is_moore:bool,
                    ltl_to_atm:LTLToAutomaton,
                    min_k:int,
                    max_k:int,
                    opt_level:int) -> str or None:
    spec = parse_acacia_and_build_expr(ltl_text, part_text, ltl_to_atm, opt_level)
    assert BAD_OUT_NAME not in (spec.inputs | spec.outputs), 'name collision'
    return _synthesize(spec, is_moore, ltl_to_atm, min_k, max_k)
...
def main():
    ...
    if not args.unreal:
        aiger_str = synthesize_real(ltl_text, part_text, is_moore, ltl_to_atm, args.minK, args.maxK, 0)
        ...
        aiger_str = synthesize_unreal(ltl_text, part_text, is_moore, ltl_to_atm, args.minK, args.maxK, 0)
        if aiger_str:
